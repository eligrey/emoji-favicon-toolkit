...
const set_emoji_favicon = (() => {
  ...
  if (is_worker) {
    ...
    self.addEventListener('message', (event: MessageEvent) => {
      const array_buffer = event.data as ArrayBuffer;
      const faviconRequest = new Request(favicon_uri);
      const faviconResponse = new Response(array_buffer, {
        headers: {
    ...
    function set_emoji_favicon(emoji: any, cacheWithServiceWorker: any): void {
      // Normalize arguments
